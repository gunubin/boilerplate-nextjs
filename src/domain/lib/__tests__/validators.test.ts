...
describe('@app/domain/lib/validators', () => {
  ...
  it('multipleKindChars()', () => {
    expect(multipleKindChars({kinds: [/a/, /b/, /c/], min: 2}).validate('a')).toBeFalsy();
    expect(multipleKindChars({kinds: [/a/, /b/, /c/], min: 2}).validate('ad')).toBeFalsy();
    expect(multipleKindChars({kinds: [/a/, /b/, /c/], min: 2}).validate('ab')).toBeTruthy();
    expect(multipleKindChars({kinds: [/a/, /b/, /c/], min: 3}).validate('abc')).toBeTruthy();
  });
